...
export function getApcaContrast(text: Rgb, background: Rgb): number {
  ...
  let lc: number;
  if (yBg > yTxt) {
    lc = (Math.pow(yBg, normBg) - Math.pow(yTxt, normTxt)) * scaleBoW;
    lc =
      lc < loBoWOffset
        ? lc - loBoWOffset * Math.pow(lc / loBoWOffset, 1.414)
        : lc;
  } else {
    lc = (Math.pow(yBg, revBg) - Math.pow(yTxt, revTxt)) * scaleWoB;
    lc =
      lc > -loWoBOffset
...
export function getAdaptiveColor(
  ...
  const candidates =
    palette === undefined || palette === null ? [BLACK, WHITE] : [...palette];
  ...
  if (algorithm === ContrastAlgorithm.wcag) {
    ...
    let bestScore = wcagContrastRatioFromLuminance(getLuminance(best), bgLum);
